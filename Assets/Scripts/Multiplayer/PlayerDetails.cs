...
public class PlayerDetails : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (view.IsMine && GameManager.Instance.gameStart == false)
        {
            ...
            {
                GameManager.Instance.SpawnBase(GridGenerator.Instance.GridIDToPosition(GridGenerator.Instance.PositionToGridID(towerGhostHitInfo.point)), faction);
            }
        }
        ...
        if (view.IsMine && GameManager.Instance.gameStart)
        {
                ...
                case CLICKSTATE.UNITCONTROL_MODE:
                    {
                        ...
                        //    Physics.Raycast(ray, out hitInfo);
                            ...
                            if (Physics.Raycast(ray, out hitInfo))
                            {
                                ...
                                {
                                    enemy.FindPath(GridGenerator.Instance.PositionToGridID(hitInfo.point));
                                }
                ...
                case CLICKSTATE.PLACE_MODE:
                    {
                        ...
                        if (SpawnGhostStructure(GameManager.Instance.basicTowerPrefab))
                        {
                            ...
                            {
                                GameManager.Instance.SpawnBasicBuilding(GridGenerator.Instance.GridIDToPosition(GridGenerator.Instance.PositionToGridID(towerGhostHitInfo.point)), faction);
                                currClickState = CLICKSTATE.UNITCONTROL_MODE;
                            }
                        }
                    }
            ...
            if (Input.GetKeyUp(KeyCode.W))
            {
                ...
                if (hitInfo.transform != null)
                    GameManager.Instance.SpawnSoldier(hitInfo.point, faction);
                Debug.Log("Spawn");
            }
        }       
    }
    ...
    public bool SpawnGhostStructure(GameObject prefab)
    {
        if (towerGhost == null)
        {
            ...
            {
                towerGhost = Instantiate(prefab, towerGhostHitInfo.point, Quaternion.identity);
        ...
        else
        {
            ...
            if (towerGhostHitInfo.transform != null)
            {
                towerGhost.transform.position = towerGhostHitInfo.point;
